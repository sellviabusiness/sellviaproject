...
| **Merchant profile** | Business name, Stripe Connect account ID | Operating a campaign, receiving payouts | `merchant_profiles` |
| **Creator profile** | Niche, audience size, engagement rate, Stripe Connect account ID | Campaign matching/discovery, receiving payouts | `creator_profiles` — note: engagement rate is currently self-reported (01. Domain Model's open fraud-implication question) |
...
| **Payment details** | **Never stored by SellVia at all** — card data goes directly to Stripe via Elements | N/A | Stripe only (04. Security → Encryption) |
...
- **Stripe** — payment/financial data, KYC information (via Stripe Connect's own onboarding, never touching SellVia's servers)
...
- Before connecting Stripe (KYC/financial onboarding): explicit notice that this information goes to Stripe specifically, not "a payment processor"
