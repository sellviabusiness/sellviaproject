...
A chargeback is initiated by the customer's bank, not by the merchant or SellVia — it comes with its own Paddle dispute flow, evidence-submission window, and a chargeback fee charged by Paddle regardless of outcome. This is a distinct process from Refund Handling and needs its own handling, not just a variant of it.

## Process

1. Paddle notifies SellVia of a dispute via webhook
2. Funds are held/reversed by Paddle pending resolution
3. Merchant (or SellVia on their behalf) can submit evidence to contest it within Paddle's window
4. If lost: treated the same as a refund for clawback purposes (per Commission Engine's 14-day rule) — plus the Paddle dispute fee itself, which isn't currently allocated to anyone in the split model

## Open Questions (genuinely unresolved, not defaults)

- **Who absorbs the Paddle dispute fee** — SellVia, the merchant, or split? Not addressed anywhere in prior docs and needs a real decision, since disputes are effectively guaranteed to happen at some volume.
...
**Founder-confirmed:** SellVia absorbs the Paddle dispute fee for a merchant's **first 5 lost chargebacks** (lifetime counter, working assumption — flag if this should reset periodically instead). From the **6th lost dispute onward**, the dispute fee is passed to the merchant, deducted from their balance.
...
On chargeback lost:
  if lifetime_disputes_lost < 5:
    SellVia absorbs the Paddle dispute fee, counter increments
  else:
