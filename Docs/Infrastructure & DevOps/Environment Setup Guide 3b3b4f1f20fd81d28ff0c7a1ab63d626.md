...
- **Payments:** Stripe Connect (test mode for Local/Staging, live for Production)
...
STRIPE_WEBHOOK_SECRET=<test mode, via Stripe CLI forwarding>
...
- [ ]  Fake/test users, fake payments (Stripe test cards), no real emails sent
- [ ]  Local Redis running for Celery + rate limiting to function at all
- [ ]  Stripe CLI forwarding webhooks to `localhost` for end-to-end checkout testing
...
- [ ]  Live Stripe keys confirmed, test keys confirmed absent
...
- **Test/live Stripe key mixup** — the single most damaging possible misconfiguration given this is a live payments system; treat key verification as a hard pre-deploy gate, not a one-time check
...
# Stripe webhook forwarding (required for local checkout testing)
...
- **CSP must explicitly allow `js.stripe.com` and `api.stripe.com`** (04. CORS, CSP & Security Headers) — an overly strict default CSP silently breaks checkout with no obvious error.
