...
- **Async:** FastAPI's native async support is a good fit here given how much of this backend is I/O-bound (Stripe API calls, webhook processing, database queries) — worth actually using `async def` route handlers and an async database driver (e.g. `asyncpg` via SQLAlchemy's async engine) rather than defaulting to sync
- **Payments:** Stripe Connect, repurposed for periodic merchant billing and creator payouts (reversed 2026-08-07, 01. Money Flow) — not a live per-sale split anymore. Stripe's Python SDK, not the Node SDK.
...
5. **Webhook handling** — FastAPI route receiving Stripe webhooks, verifying signature, enqueuing background work (see Background Jobs — also updated for Python)
...
## Why Stripe Connect (unchanged reasoning)

Same as before — managed KYC/tax-form collection and money movement via Stripe Connect Express accounts is far less engineering/compliance surface than a custom ledger + manual payouts, regardless of backend language.

## Open Questions

- **Stripe Connect account type:** still recommend Express for MVP (unchanged from before — this decision doesn't depend on backend language)
...
- **Database, Redis, Stripe, and Ory Kratos are already separate, independently-managed services** — none of them go down because of a bug in SellVia's own code.
