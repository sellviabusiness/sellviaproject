...
**Double-charge prevention, restated:** Paddle redelivers webhooks by design (not a bug to guard against, an expected behavior to design for) u2014 idempotent processing means a redelivered `transaction.completed` for the same Paddle transaction ID is a no-op the second time, never a second credit. Already required (Webhook Security, Event-Driven Architecture); this is the same rule, not a new one.