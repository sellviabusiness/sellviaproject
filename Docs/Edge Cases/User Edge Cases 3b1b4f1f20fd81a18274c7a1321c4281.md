...
- **Creator's Stripe Connect onboarding incomplete** — if a Creator is approved for a campaign but hasn't finished Stripe Express onboarding, they shouldn't be able to generate a live AffiliateLink yet (a link with no way to receive payout is a trust problem, not just an inconvenience). Needs an explicit "onboarding incomplete" gate before link activation.

## Open Questions

- Exact UX for the Stripe-onboarding-incomplete gate (blocked entirely vs. link works but payout is held) — recommend blocking link activation entirely rather than accruing unpayable commission, to avoid a confusing backlog
